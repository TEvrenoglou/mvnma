...
#' The mvNMA model supported by this package refers to the single correlation coefficient model, 
#' interpreted as an amalgam of within- and across-outcome correlations. In this way, the model does 
#' not depend on the extraction of within-study outcome correlations, which are seldom reported at the 
#' study level. The treatment effect estimates and confidence intervals can be summarised both in terms of 
#' per-outcome treatment hierarchies and in terms of an across-outcomes benefit-risk assessment. 
#' The former is possible using ranking methods such as SUCRA, probability of best, and median (or mean) ranks, 
#' each accompanied by a credible interval. A benefit-risk assessment is possible through the VIKOR method. This approach, 
#' originally proposed in the field of multi-criteria decision analysis, uses a deterministic algorithm to 
#' provide an amalgamated treatment hierarchy across outcomes and explicitly identify the set of treatments 
#' that offer the best compromise between benefits and harms across all outcomes. Since the output of the 
#' method relies on Markov Chain Monte Carlo (MCMC) sampling, convergence can be checked using a series of 
#' diagnostics, including trace plots, density plots, and the R-hat statistic. Finally, this package offers 
#' the option to visualise the results of the mvNMA model through forest plots, which display the treatment 
...
#'  \item Function \code{\link{spie.chart}} to calculate a between outcomes hierarchy using the spie charts method.
#' }
#' 
