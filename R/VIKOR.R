#' Rank treatments across all outcomes using the VIKOR multi-criteria decision analysis method.
#' 
#' @description
...
#' @param weights Outcome weights. The weights should always sum to 1. If not then they are standardized. If NULL, the function will assume equal outcome weights. 
#' @param v A scalar from 0 to 1 interpreted as the weight of the decision making process. Following guidance from the multi-criteria decison analysis field it is set to 0.5.
#' @param pos.sol A vector of values containing the positive ideal solution for each outcome. If NULL it is always set to 1 for each outcome (see details). 
#' @param neg.sol A vector of values containing the negative ideal solution for each outcome. If NULL it is always set to 0 for each outcome (see details). 
#'
#' @details
...
vikor.mvnma <- function (x, weights=NULL,
  ...
  if(inherits(x,"mvrank")){
    ...
    # x <- rbind(d1_new$SUCRA,d2_new$SUCRA)
    
    row.names(rankings) <- t
...
minmax <- rep("max",ncol(rankings))

if(is.null(pos.sol)){
  
  pos.sol <- rep(1,ncol(rankings))  
...
if(is.null(weights)){
  ...
  weights <- rep(1/ncol(rankings), ncol(rankings))

}else if((!is.null(weights) & (length(weights)!=ncol(rankings)))){
...
if((!is.null(weights) & (sum(weights)!=1))){
  ...
  new_weights <- paste(weights,collapse = ", ")
  
  message <- paste("Weights should always sum up to 1. To do so the given weights are now standardized and the new weights are: ",new_weights,sep = "")
  
  warning(message)
  
}
  critno <- ncol(rankings)
  
  altno <- nrow(rankings)
  
  ## 
  
  dist <- matrix(ncol=ncol(rankings),nrow = nrow(rankings),
                 dimnames = list(row.names(rankings),colnames(rankings))
                 )
  
  for (i in 1:critno) dist[, i] <- (pos.sol[i] - rankings[,i])/(pos.sol[i] - neg.sol[i])
  ...
  names(Q) <- row.names(S) <- row.names(R) <- row.names(rankings)
...
################################################ 
  ...
  class(results) <- c("vikor.mvnma",class(results))
  
  attr(results,"performance.table") <- rankings
